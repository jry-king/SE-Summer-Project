...
def embed(experiment_root, dataset, filename, image_root):
    os.system("python embed.py --quiet --experiment_root " + experiment_root + " --dataset " + dataset
...
def evaluation(excluder, query_dataset, query_embeddings, gallery_dataset, gallery_embeddings, metric, batch_size):
    ...
    query_pids, query_fids = common.load_dataset(query_dataset, None)
    gallery_pids, gallery_fids = common.load_dataset(gallery_dataset, None)
    ...
    excluder = import_module('excluders.' + excluder).Excluder(gallery_fids)
